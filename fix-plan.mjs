...
// Get all plans
const plans = db.prepare('SELECT id, title, status, current_task_id as currentTaskId FROM plans ORDER BY created_at DESC LIMIT 10').all();
...
for (const plan of failedPlans) {
  ...
  const tasks = db.prepare('SELECT id, title, status FROM plan_tasks WHERE plan_id = ? ORDER BY "order"').all(plan.id);
  ...
  if (plan.currentTaskId) {
    ...
    if (currentTask) {
      console.log(`Current blocking task: "${currentTask.title}" - ${currentTask.status}`);

      if (currentTask.status === 'completed') {
        console.log('⚠️  FIXING: Current task is completed but plan is failed!');
        db.prepare('UPDATE plans SET status = ?, updated_at = ? WHERE id = ?').run('paused', Date.now(), plan.id);
  ...
  if (stats.failed === 0 && stats.running === 0 && stats.pending > 0) {
    ...
    db.prepare('UPDATE plans SET status = ?, current_task_id = NULL, updated_at = ? WHERE id = ?').run('paused', Date.now(), plan.id);
    console.log('✅ Fixed! Plan set to paused.');
  }
}
