...
test.describe('Dashboard – Real-time Features', () => {
  ...
  test('connection status indicator is visible', async ({ page }) => {
    ...
    if (!isVisible) {
      console.log(
        'Connection indicator not visible – may be hidden without an active repo'
      );
    }
  });

  test('shows empty state or repository selector when no repo is selected', async ({
    page,
  }) => {
    ...
    const isVisible = await emptyOrSelector
      .first()
  ...
  test('handles SSE connection errors gracefully (no crash)', async ({
    page,
  }) => {
  ...
  test('reconnecting banner appears after SSE failure', async ({ page }) => {
    ...
    const bannerVisible = await reconnectBanner
      .first()
...
test.describe('Dashboard – Task Timeline', () => {
  ...
  test('task status badges use semantic ARIA when present', async ({
    page,
  }) => {
    await gotoDashboard(page);

    const badges = page.locator(
      '[role="status"], [data-status], [aria-label*="status" i]'
    );
  ...
  test('tasks tab content renders after tab click', async ({ page }) => {
    ...
    const tasksTab = page
      .locator('[role="tab"]:has-text("Tasks"), button:has-text("Tasks")')
      .first();
    ...
    if (tabVisible) {
      ...
      const tabContent = page
        .locator('[role="tabpanel"], [data-testid="tasks-content"]')
        .first();
      const panelVisible = await tabContent
        .isVisible({ timeout: 5_000 })
        .catch(() => false);
...
test.describe('Dashboard – Session Controls', () => {
  test('session controls bar is accessible when session is active', async ({
    page,
  }) => {
    await gotoDashboard(page);
  ...
  test('session history modal opens and closes', async ({ page }) => {
    ...
    const historyBtn = page
      .locator('button:has-text("History"), button[aria-label*="history" i]')
      .first();
    const historyBtnVisible = await historyBtn
      .isVisible({ timeout: 5_000 })
      .catch(() => false);
