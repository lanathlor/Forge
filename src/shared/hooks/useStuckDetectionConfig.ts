...
async function updateConfigOnServer(newConfig: StuckDetectionConfig): Promise<StuckDetectionConfig> {
  const response = await fetch('/api/stuck-detection-config', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newConfig) });
  if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'Failed to update configuration'); }
  return (await response.json()).config;
}

async function resetConfigOnServer(): Promise<StuckDetectionConfig> {
  const response = await fetch('/api/stuck-detection-config', { method: 'DELETE' });
...
export function useStuckDetectionConfig(): UseStuckDetectionConfigReturn {
  ...
  const fetchConfig = useCallback(async () => {
    try { setLoading(true); setError(null); setConfig(await fetchConfigFromServer()); }
  ...
  const updateConfig = useCallback(async (updates: Partial<StuckDetectionConfig>): Promise<boolean> => {
    ...
    try { setError(null); setConfig(await updateConfigOnServer({ ...config, ...updates })); return true; }
  ...
  const resetConfig = useCallback(async (): Promise<boolean> => {
    try { setError(null); setConfig(await resetConfigOnServer()); return true; }
    catch (err) { setError(err instanceof Error ? err.message : 'Unknown error'); return false; }
  }, []);
