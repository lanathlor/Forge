...
export function PlanPreflightBanner({
  ...
  return (
    <div
      className={cn(
        'rounded-lg border px-3 py-2 transition-all animate-in fade-in slide-in-from-top-2 duration-300',
        allPassed && 'border-emerald-200 bg-emerald-50/50 dark:border-emerald-900 dark:bg-emerald-950/20',
        hasFailures && 'border-red-200 bg-red-50/50 dark:border-red-900 dark:bg-red-950/20',
        !allPassed && !hasFailures && 'border-amber-200 bg-amber-50/50 dark:border-amber-900 dark:bg-amber-950/20',
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium">
          {isChecking ? 'Running pre-flight checks...'
            : allPassed ? 'All systems go'
            : hasFailures ? 'Pre-flight check failed'
            : 'Ready with warnings'}
        </span>
      </div>
