  ...
  const processedPlans = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      if (statusFilter === 'running') {
        ...
        filtered = filtered.filter((p) => p.status === 'running' || p.status === 'paused');
      } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6">
        ...
        <p className="text-sm text-muted-foreground text-center max-w-sm mb-6">
          Plans help you break down complex tasks into phases and track progress.
          Generate one with Claude or create your own.
        </p>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40">
                ...
                {SORT_OPTIONS.map((opt) => (
                  <DropdownMenuItem key={opt.value} onClick={() => toggleSort(opt.value)}>
                    {opt.label}
                    {sortField === opt.value && (
