...
const StatusDot = memo(
  ({ status, size = 'sm' }: { status: ClaudeStatus; size?: 'xs' | 'sm' }) => {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isFocused &&
          'animate-slide-in-right bg-muted/80 ring-1 ring-primary/30',
        isSelected && !isFocused && 'bg-primary/8 border-l-2 border-l-primary',
        needsAttention && 'animate-pulse-alert ring-1 ring-red-400/50'
      )}
    >
      {/* Status indicator */}
      <div
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {needsAttention && (
            <span className="shrink-0 rounded bg-red-100 px-1 py-0.5 text-[9px] font-bold text-red-700 dark:bg-red-900/40 dark:text-red-300">
              !
            </span>
          )}
        </div>
      ...
      <div className="flex shrink-0 items-center gap-1">
        {shortcutNum && shortcutNum <= 9 && (
          <span className="hidden items-center gap-0.5 rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground group-hover/selector:flex">
...
interface RepoListSectionProps {
  filteredItems: { active: Repository[]; all: Repository[] };
  flatList: Repository[];
  searchQuery: string;
  liveStateMap: Map<string, RepoSessionState>;
  currentSelectedId: string | null;
  focusIndex: number;
  getAlertForRepo: (id: string) => StuckAlert | null;
  onSelect: (repo: Repository) => void;
  onHover?: (repoId: string) => void;
  onSetFocusIndex: (idx: number) => void;
  onClearSearch: () => void;
  listRef: RefObject<HTMLDivElement | null>;
}

function RepoListSection({ filteredItems, flatList, searchQuery, liveStateMap, currentSelectedId, focusIndex, getAlertForRepo, onSelect, onHover, onSetFocusIndex, onClearSearch, listRef }: RepoListSectionProps) {
  return (
    <div ref={listRef} id="repository-list" className="flex-1 overflow-y-auto p-1.5" role="region" aria-label="Repository list">
      {filteredItems.active.length > 0 && (
        <div className="animate-slide-in-down mb-2">
          <SectionHeader title="Active Work" count={filteredItems.active.length} icon={Activity} />
          <div className="stagger-children flex flex-col gap-0.5" role="list" aria-label="Active repositories">
            {filteredItems.active.map((repo, i) => {
              const live = liveStateMap.get(repo.id);
              const alert = getAlertForRepo(repo.id);
              return (
                <ActiveRepoRow key={repo.id} repo={repo} liveState={live} isSelected={currentSelectedId === repo.id} isFocused={focusIndex === i} shortcutNum={i + 1} needsAttention={Boolean(alert && !alert.acknowledged)} onSelect={() => onSelect(repo)} onMouseEnter={() => { onSetFocusIndex(i); onHover?.(repo.id); }} />
              );
            })}
          </div>
        </div>
      ...
      {filteredItems.all.length > 0 && (
        <div className="animate-slide-in-down">
          ...
          <div className="stagger-children flex flex-col gap-0.5" role="list" aria-label="All repositories">
            {filteredItems.all.map((repo, j) => {
              const globalIdx = filteredItems.active.length + j;
              const live = liveStateMap.get(repo.id);
              return (
                <RepoRow key={repo.id} repo={repo} liveState={live} isSelected={currentSelectedId === repo.id} isFocused={focusIndex === globalIdx} onSelect={() => onSelect(repo)} onMouseEnter={() => { onSetFocusIndex(globalIdx); onHover?.(repo.id); }} />
              );
            })}
          </div>
        </div>
      ...
      {flatList.length === 0 && searchQuery && (
        <div className="flex flex-col items-center justify-center py-8 text-center" role="status">
          <Search className="mb-2 h-5 w-5 text-muted-foreground/40" aria-hidden="true" />
          <span className="text-sm text-muted-foreground">No repos match &quot;{searchQuery}&quot;</span>
          <button onClick={onClearSearch} className="mt-2 rounded px-2 py-1 text-xs text-primary hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" aria-label="Clear search and show all repositories">
            Clear search
          </button>
        </div>
      )}
...
export function RepositorySelector({
  ...
  const { activeRepos, allRepos } = useMemo(() => {
    ...
    active.sort((a, b) => {
      ...
      if (!la || !lb) return 0;
      if (la.needsAttention !== lb.needsAttention)
        return la.needsAttention ? -1 : 1;
      const diff =
        STATUS_CONFIG[la.claudeStatus].priority -
        STATUS_CONFIG[lb.claudeStatus].priority;
      if (diff !== 0) return diff;
      return (
        new Date(lb.lastActivity).getTime() -
        new Date(la.lastActivity).getTime()
      );
    });
  ...
  const filteredItems = useMemo((): {
    ...
    const filteredActive: Repository[] = scored
      .filter((s: { repo: Repository }) => activeIds.has(s.repo.id))
      .map((s: { repo: Repository }) => s.repo);
    const filteredAll: Repository[] = scored
      .filter((s: { repo: Repository }) => !activeIds.has(s.repo.id))
      .map((s: { repo: Repository }) => s.repo);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key >= '1' && e.key <= '9') {
        ...
        const combinedTop = [
          ...activeRepos,
          ...allRepos.filter((r) => !activeRepos.find((ar) => ar.id === r.id)),
        ];
  ...
  if (isCollapsed) {
    return (
      <Card className="flex h-full flex-col items-center gap-2 py-3">
        <button
  ...
  return (
    <Card className="group/selector flex h-full flex-col overflow-hidden">
      {/* Header with search */}
      <div className="flex-shrink-0 border-b border-border/50 p-2">
        <div className="mb-2 flex items-center gap-2 px-1">
          ...
          <button
            ...
            className="rounded p-1 transition-colors hover:bg-muted/60 disabled:opacity-50"
          >
            <RefreshCw
              className={cn(
        ...
        <div className="relative">
          ...
          <input
            ...
            className={cn(
              'h-8 w-full rounded-md border pl-8 pr-12 text-sm',
              'border-border/50 bg-muted/30',
              'placeholder:text-muted-foreground/50',
              'focus:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary/40',
              'transition-all duration-150'
            )}
          />
          <kbd
            className="absolute right-2 top-1/2 hidden -translate-y-1/2 items-center gap-0.5 rounded border border-border/50 bg-muted px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground sm:inline-flex"
      ...
      <RepoListSection
        ...
        getAlertForRepo={getAlertForRepo}
