...
const STATUS_EVENT_MAP: Record<
  string,
  { type: RepoEvent['type']; label: string }
> = {
...
function mapRepoToSnapshot(
  repo: RepoSessionState
): Partial<RepoSnapshot> & { repositoryId: string } {
  return {
...
function buildEventFromTaskUpdate(
  data: TaskUpdateData
): { type: RepoEvent['type']; message: string } | null {
...
async function loadSessionStats(
  ...
  const res = await fetch(`/api/sessions?repositoryId=${repoId}`);
  if (!res.ok) return;
  ...
  dispatch(updateSnapshot({ repositoryId: repoId, sessionStats: stats }));
}

function useSessionStatsFetcher(
  currentRepoId: string | null,
  dispatch: AppDispatch
) {
  ...
  useEffect(() => {
    ...
    loadSessionStats(currentRepoId, dispatch).catch(() => {
...
export function useRepoSnapshot({
  repositories,
  currentRepoId,
}: UseRepoSnapshotOptions): UseRepoSnapshotReturn {
  const dispatch = useAppDispatch();
  const snapshots = useAppSelector((state) => state.repoSnapshot.snapshots);
  const dismissedRepoId = useAppSelector(
    (state) => state.repoSnapshot.dismissedRepoId
  );
