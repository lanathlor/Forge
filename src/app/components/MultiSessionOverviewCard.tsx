...
function computeStats(repos: RepoSessionState[]): AggregateStats {
  ...
  const waitingCount = repos.filter(
    (r) => r.claudeStatus === 'waiting_input'
  ).length;
  const workingCount = repos.filter(
    (r) => r.claudeStatus === 'thinking' || r.claudeStatus === 'writing'
  ).length;
  const idleCount = repos.filter(
    (r) => r.claudeStatus === 'idle' || r.claudeStatus === 'paused'
  ).length;
  const activeRepos = repos.filter(
    (r) => r.sessionStatus === 'active' || r.sessionStatus === 'paused'
  ).length;
  const reposNeedingAttention = repos.filter(
    (r) =>
      r.needsAttention ||
      r.claudeStatus === 'stuck' ||
      r.claudeStatus === 'waiting_input'
  ).length;
  ...
  for (const repo of repos) {
    if (repo.currentTask) {
      ...
      if (
        ...
        repo.currentTask.status === 'approved'
      ) {
        completedTasks += 1;
      }
    }
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-[10px] text-muted-foreground">
        ...
        <span className="tabular-nums">
          {stats.activeRepos} active of {stats.totalRepos}
        </span>
      </div>
...
function RepoMiniCard({
  ...
  const isWorking =
    repo.claudeStatus === 'thinking' || repo.claudeStatus === 'writing';
  const needsAction =
    repo.claudeStatus === 'stuck' || repo.claudeStatus === 'waiting_input';
  ...
  return (
    ...
    >
      ...
      >
        <Icon
          className={cn('h-4 w-4', style.color, isWorking && 'animate-pulse')}
        />
        {isWorking && (
          <span
            className={cn(
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-xs text-muted-foreground">
          {repo.currentTask
            ? truncate(repo.currentTask.prompt, 50)
            : 'No active task'}
        </p>
      </div>
...
export const MultiSessionOverviewCard = React.memo(
  function MultiSessionOverviewCard({
    onSelectRepo,
    className,
    ...
    const visibleRepos = useMemo(
      () => (isExpanded ? sortedRepos : sortedRepos.slice(0, 3)),
      [isExpanded, sortedRepos]
    );
            ...
            <>
              ...
              {stats.reposNeedingAttention > 0 && (
                ...
                >
                  ...
                  <span>
                    <strong>{stats.reposNeedingAttention}</strong> repo
                    {stats.reposNeedingAttention !== 1 ? 's' : ''} need
                    {stats.reposNeedingAttention === 1 ? 's' : ''} your
                    attention
                  </span>
                </div>
              )}
